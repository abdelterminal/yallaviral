...
export function StudioDialog({ resource }: StudioDialogProps) {
    ...
    const form = useForm({
        defaultValues: {
            ...
            tags: resource?.tags?.join(", ") || "", // Amenities handled as comma-separated tags
        },
    });
    ...
    const onSubmit = async (data: any) => {
        try {
            ...
            toast.success(resource ? "Studio updated" : "Studio added");
            setOpen(false);
            if (!resource) form.reset();
            router.refresh();
        } catch (error) {
            toast.error("Error", { description: "Could not save studio." });
        }
    };
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <div className="flex justify-end pt-4">
                            <Button type="submit" disabled={uploading} className="font-bold">
                                {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : (resource ? "Update Studio" : "Add Studio")}
                            </Button>
                        </div>
                    </form>
                </Form>
