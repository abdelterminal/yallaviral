...
export function ResourceCard({ resource }: ResourceCardProps) {
    ...
    return (
        <Card className="group overflow-hidden rounded-[2.5rem] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.1)] border-0 flex flex-col md:flex-row h-auto md:h-[340px] bg-card shadow-[0_10px_40px_-10px_rgba(0,0,0,0.06)]">
            {/* Image Section (Left) */}
            <div className="w-full md:w-2/5 relative p-4 md:pr-0 h-[240px] md:h-full">
                <div className="w-full h-full relative overflow-hidden rounded-[2rem] bg-muted shadow-inner">
                    {resource.image_url ? (
                        ...
                        <div className="flex h-full items-center justify-center text-muted-foreground/50 font-bold uppercase tracking-wider text-xs">
                            No Image
                        </div>
                    )}
                </div>
            ...
            <div className="flex flex-1 flex-col md:flex-row">
                ...
                <div className="flex-1 p-8 flex flex-col relative justify-center">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {resource.tags?.map(tag => {
                            ...
                            return (
                                <div key={tag} className="flex items-center gap-2 px-4 py-2 rounded-full bg-muted text-xs font-bold text-muted-foreground hover:bg-primary/10 hover:text-primary transition-all cursor-default">
                ...
                <div className="p-8 border-0 bg-muted/30 flex flex-row md:flex-col items-center md:justify-center justify-between gap-6 min-w-[240px]">
                    ...
                    <BookingSheet resource={resource}>
                        <Button size="lg" className="w-full h-14 font-black text-lg">
                            Book This Set
                        </Button>
                    </BookingSheet>
                </div>
